...
public class KowaxTestDisplayManager implements DisplayManager, HttpHandler {
	...
	public class DisplaySession {
		...
		public DisplaySession(TokenKey tokenKey, String username) {
			stdio = new Stdio(null, null);
			session = new Session(stdio);
			session.setAuthenticated(true);
			session.setUsername(username);
			commandRunner = new CommandRunner(tokenKey, false);
		}
		
	}

	public KowaxTestDisplayManager(TokenKey tokenKey) {
	...
	public void handle(HttpExchange arg0) throws IOException {
		...
		myResponse.append("</head><body>");
		myResponse.append("<h1>" + Initrfs.SHELLNAME + " " + Initrfs.VERSION + " - Kowax Test Display Manager (Mark 1)</h1>");
		...
		myResponse.append("</ul>");
		myResponse.append("<h7>Powered by XSpaceSoft 2008-2015</h7>");
